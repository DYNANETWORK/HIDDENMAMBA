...
import HIDDENMAMBA.modules.sql.feds_sql as sql
from HIDDENMAMBA import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher, MAMBA_STAR
from HIDDENMAMBA.modules.disable import DisableAbleCommandHandler
from HIDDENMAMBA.modules.helper_funcs.alternate import send_message
from HHIDDENMAMBA.modules.helper_funcs.chat_status import is_user_admin
from HIDDENMAMBA.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from HIDDENMAMBA.modules.helper_funcs.string_handling import markdown_parser
...
def del_fed(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        if getinfo is False:
            ...
            return
        if int(getinfo["owner"]) == int(user.id) or int(user.id) == OWNER_ID or int(user.id) == MAMBA_STAR:
            fed_id = is_fed_id
        else:
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if user_id == OWNER_ID or user_id == MAMBA_STAR:
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in DRAGONS or int(import_userid) == LEGENDX22:
                        failed += 1
                        continue
            ...
            with open(
                ...
                for data in reader:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in DRAGONS or int(import_userid) == MAMBA_STAR:
                        failed += 1
                        continue
                    if int(import_userid) in TIGERS:
                        failed += 1
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if fed_admins is False:
        return False
    if int(user_id) in fed_admins or int(user_id) == OWNER_ID or int(user_id) == LEGENDX22:
        return True
    else:
        return False
...
def is_user_fed_owner(fed_id, user_id):
    ...
    if str(user_id) == getfedowner or int(user_id) == OWNER_ID or int(user_id) == LEGENDX22:
        return True
    else:
        return False
